...
long Encode1count_distance=0;
long Encode2count_distance=0;
extern float Motor1Speed = 0.00;
extern float Motor2Speed = 0.00;
...
void EXTI15_10_IRQHandler(void)
{
  ...
  HAL_GPIO_EXTI_IRQHandler(Rightobstacle_inter_Pin);
...
{
	if(htim == &htim1)//htim1 500HZ  2ms 中断�????????????�????????????
	{
		TimerCount++;
		timer++;
		if(TimerCount %5 == 0)//�????????????10ms执行�????????????�????????????
		{
				  Encode1Count = (short)__HAL_TIM_GET_COUNTER(&htim8);
				  Encode1count_distance+=Encode1Count;
				  Encode2Count = (short)__HAL_TIM_GET_COUNTER(&htim4);
				  Encode2count_distance+=Encode2Count;
				  __HAL_TIM_SET_COUNTER(&htim8,0);
				  __HAL_TIM_SET_COUNTER(&htim4,0);

				  Motor1Speed = (float)Encode1Count*100/(4*30*11);
				  Motor2Speed = (float)Encode2Count*100/(4*30*11);
